...
fl_base <- ggplot(data = states) +
  ...
  coord_fixed(1.3) + 
  guides(fill=FALSE) +
  geom_polygon(data = fl_df, mapping = aes(x = long, y = lat, group = group), fill = "#00cc66") + 
  geom_count(data = florida_together, aes(srch_destination_longitude, srch_destination_latitude, color = "red", size = n))
...
>>>>>>> ab6963b87f1e46ccad466c17b332a53ed0a69043
states <- map_data("state")

usa <-map_data("usa")

<<<<<<< HEAD
california_together <- top_five_dest %>%
  left_join(new_table, top_five_dest, by = "srch_destination_name") %>%
...
ca_base <- ggplot(data = states) +
  ...
  geom_count(data = california_together, aes(srch_destination_longitude, srch_destination_latitude, color = "red", size = n))
...
ny_base <- ggplot(data = states) +
  geom_polygon(aes(x=long, y = lat, group = group), color = 'white', fill = "lightgray") + 
  coord_fixed(1.3) + 
  guides(fill=FALSE) +
  geom_polygon(data = ny_df, mapping = aes(x = long, y = lat, group = group), fill = "#6600cc") + 
  geom_count(data = ny_together, aes(srch_destination_longitude, srch_destination_latitude, size = n), color = "pink")
...
il_base <- ggplot(data = states) +
  ...
  coord_fixed(1.3) + 
  guides(fill=FALSE) +
  geom_polygon(data = IL_df, mapping = aes(x = long, y = lat, group = group)) + 
  geom_count(data = IL_together, aes(srch_destination_longitude, srch_destination_latitude, size = n), colour = "yellow")
