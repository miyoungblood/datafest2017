...
ca_base <- ggplot(data = states) +
  ...
  coord_fixed(1.3) + 
  guides(fill=FALSE) +
  geom_polygon(data = ca_df, mapping = aes(x = long, y = lat, group = group), fill = "#00cc66") + theme_void() + 
  geom_count(data = beta_together, aes(srch_destination_longitude, srch_destination_latitude, color = "red"))
